...
class SpheroNode(object):
    ...
    # connecting with the sphero, configure colission
    def start(self):
        try:
...
#       # now start receiving packets
    ...
    # Read the datastream from the sphero
    ...
    def parse_collision(self, data):
        if self.is_connected:
            ...
            # Calculating the collision angle with y-axis as the reference
            self.angle = math.atan2(self.cx, self.cy)*180/math.pi
            # self.angle = self.angle + 90
            if math.fabs(self.angle) <= 90:
                self.theta = 90 - math.fabs(self.angle)
                self.head = self.head - \
                    math.copysign(1, self.angle)*2*self.theta

                if math.fabs(self.head) > 180:
                    self.head = self.head - math.copysign(1, self.head) * 360

                print '[cx:{0}],[cy:{1}],[an:{2}],[hd:{3}]'.format(
                    self.cx, self.cy, self.angle, self.head, self)

                self.collipy = np.vstack([self.collipy, np.matrix(
                    [self.cx, self.cy, self.theta, self.head, self.angle, self.cspeed])])
    ...
    # send Bluetooth package with movement instruction
